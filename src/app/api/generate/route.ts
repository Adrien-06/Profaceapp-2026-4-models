...
const MODEL_PROMPTS: Record<ModelId, string> = {
  ...
  innovator:
    'A dating picture three-quarter right studio portrait from this selfie,  arms crossed, torso position slightly turned on his/her right side, looking directly into the camera with Maintaining the natural mouth shape with slight smile and exact facial expression from the reference image. No added teeth, no exaggerated smile. Wearing a premium material white shirt with. Shot on a high-end medium format camera, 85mm lens, f/2.8 aperture, creating a soft cinematic bokeh. The lighting is soft and directional (clamshell or light Rembrandt style). Clean, Sophisticated premium. MINIMALIST STREET ART WALL BEHIND, with a cinematic bokeh blur, brightened soft key light on the subject, well-defined rim light to separate the subject from the background Photorealistic, hyper-detailed skin texture, with slightly smoothing out the imperfections, tanned skin, individual hair strands, high-resolution 8k, commercial advertising photography style. ',
};
